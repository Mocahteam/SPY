...
public class MapDesc : FSystem
{
    ...
    private void updateMap(GameObject unused)
    {
        ...
        foreach (GameObject door in f_doors)
        {
            ...
            exportMap[pos.x, pos.y] += "+("+ gameDataLoc.localization[42] + door.GetComponent<ActivationSlot>().slotID+" "+(door.activeInHierarchy ? gameDataLoc.localization[43] : gameDataLoc.localization[44]) + ")";
        }
        ...
        if (EventSystem.current.currentSelectedGameObject)
            Debug.Log(EventSystem.current.currentSelectedGameObject.name+ " "+ childSelectedPos);
        ...
        for (int j = 0; j < exportMap.GetLength(1); j++)  {
            string line = gameDataLoc.localization[47] + " " + j + ": "; // Ligne X:

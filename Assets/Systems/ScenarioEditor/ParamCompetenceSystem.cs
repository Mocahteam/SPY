...
public class ParamCompetenceSystem : FSystem
{
	...
	public GameObject CompetencyToggle; // Prefab de l'affichage d'une compétence
	...
	public void filterCompatibleLevels(bool nameFiltering) { 
		...
		if (gameData.selectedScenario == UtilityLobby.testFromScenarioEditor)
		{
			...
			showLevelInfo(Utility.extractFileName(dl.filePath), dl);
	...
	private void loadScenario(string scenarioKey)
    {
		if (gameData.scenarios.ContainsKey(scenarioKey))
		{
			...
			foreach (DataLevel levelPath in gameData.scenarios[scenarioKey].levels)
			{
				...
				newLevel.GetComponentInChildren<TMP_Text>().text = Utility.extractFileName(levelPath.filePath);
	...
	private void showLevelInfo(string path, DataLevel overridedData = null) { 
		...
		if (gameData.levels.ContainsKey(absolutePath))
		{
			...
			if (levelSelected != null)
			{
				...
				{
					dlb.data.filePath = absolutePath;
					dlb.data.missionName = Path.GetFileNameWithoutExtension(path);
	...
	private string buildScenarioContent()
    {
		...
		foreach (Transform child in contentScenario.transform)
		{
			...
			if (dataLevel.missionName == null || dataLevel.missionName == "")
				dataLevel.missionName = Path.GetFileNameWithoutExtension(dataLevel.filePath);
			scenarioExport += "\t<level src=\"" + child.GetComponentInChildren<TMP_Text>().text + "\" name=\"" + dataLevel.missionName + "\"";
	...
	private void DebugLogLevelsCompetencies()
    {
		...
		for (int c = 0; c < f_UI_competencies.Count; c++)
			csvExport += "\t" + Utility.extractLocale(f_UI_competencies.getAt(c).GetComponent<Competency>().id);
		csvExport += "\n";
		// parse all levels
		foreach (KeyValuePair<string, XmlNode> level in gameData.levels)
		{
